function fig = figS03a_prototype_purity(cfg)
%FIGS03A_PROTOTYPE_PURITY Prototype purity versus mask area.
if nargin < 1 || isempty(cfg), cfg = setup_portable_paths(); end
fig = plot_failure_metric(cfg, 'purity', ...
    '(a) Prototype purity versus mask area', 'prototype purity');
end

function fig = plot_failure_metric(cfg, metric, titleText, yLabelText)
...
for m = 1:numel(methods)
    ...
    [maskRatio, order] = sort(T.mask_ratio(rows)); values = T.(metric)(rows);
...
xlabel(ax, 'mask area (% of image)'); ylabel(ax, yLabelText); title(ax, titleText);
