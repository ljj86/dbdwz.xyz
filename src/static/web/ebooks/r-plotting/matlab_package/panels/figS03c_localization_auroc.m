function fig = figS03c_localization_auroc(cfg)
%FIGS03C_LOCALIZATION_AUROC Localization AUROC versus mask area.
...
for m = 1:numel(methods)
    ...
    [maskRatio, order] = sort(T.mask_ratio(rows)); values = T.pixel_auroc(rows);
...
xlabel(ax, 'mask area (% of image)'); ylabel(ax, 'pixel AUROC');
title(ax, '(c) Localization AUROC versus mask area');
